...
const IdDocumentContent = () => {
    ...
    if (user.email !== '') {
      ...
      const getData = async () => {
        ...
        if (filteredData[0].id !== '') {
          ...
          if (filteredData[0].idDocumentImg !== '') {
            setImgFromFirestore(filteredData[0].idDocumentImg)
          }
        }
      }
      getData()
  ...
  const updateItem = async (item) => {
    try {
      ...
      const newFields = { idDocumentImg: item }
  ...
  return (
    <div>
      <div className='py-5 px-3'>
        {!imgFromFirestore ? (
          <h4 className='rubik-400 text-center mb-4 m-0 p-0'>ID Document</h4>
        ) : (
          ''
        )}
        {fileLoading && (
...
export default IdDocumentContent
