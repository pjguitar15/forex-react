import React, { useEffect, useState } from 'react'
import { useNavigate, useLocation } from 'react-router-dom'
import { Button, Container } from 'react-bootstrap'
import useGetDataFromEmail from '../../custom-hooks/useGetDataFromEmail'
import MySpinner from '../../components/MySpinner'
// assets
import topBg from '../../assets/forex-cross-rate-bg.jpg'

const TopButtons = () => {
  const [data, loading] = useGetDataFromEmail()
  const [logoutLoading, setLogoutLoading] = useState(false)
  const [windowDimension, detectHW] = useState({
    winWidth: window.innerWidth,
    winHeight: window.innerHeight,
  })

  const detectSize = () => {
    detectHW({
      winWidth: window.innerWidth,
      winHeight: window.innerHeight,
    })
  }
  const navigate = useNavigate()
  const location = useLocation()
  const logoutHandler = () => {
    setLogoutLoading(true)
    setTimeout(() => {
      setLogoutLoading(false)
      sessionStorage.removeItem('Auth Token')
      navigate('/login')
    }, 2000)
  }

  useEffect(() => {
    if (localStorage.getItem('userSegment') !== '') {
      if (data.segment) {
        localStorage.setItem('userSegment', data.segment)
      }
    }
  }, [data])
  return (
    // background-position: center !important;
    // background-size: cover !important;
    <div className='top-btn'>
      <div>
        <Container className='col-12 w-100 mx-auto d-flex justify-content-between py-5'>
          <h2 className='m-0 border border-light text-light p-3 lead col-lg-3 rounded border-2 px-4 manrope-600'>
            Segment:
            <div className='fw-bold display-6 text-light'>
              $
              {localStorage.getItem('userSegment') || data.segment + ' Segment'}
            </div>
          </h2>
          <div>
            <Button
              disabled={logoutLoading}
              variant='light'
              className='rubik-400 my-1 border-0'
              size='sm'
              onClick={logoutHandler}
            >
              <svg
                xmlns='http://www.w3.org/2000/svg'
                width='16'
                height='16'
                fill='currentColor'
                className='bi bi-box-arrow-left me-2'
                viewBox='0 0 16 16'
              >
                <path
                  fill-rule='evenodd'
                  d='M6 12.5a.5.5 0 0 0 .5.5h8a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5h-8a.5.5 0 0 0-.5.5v2a.5.5 0 0 1-1 0v-2A1.5 1.5 0 0 1 6.5 2h8A1.5 1.5 0 0 1 16 3.5v9a1.5 1.5 0 0 1-1.5 1.5h-8A1.5 1.5 0 0 1 5 12.5v-2a.5.5 0 0 1 1 0v2z'
                />
                <path
                  fill-rule='evenodd'
                  d='M.146 8.354a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L1.707 7.5H10.5a.5.5 0 0 1 0 1H1.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3z'
                />
              </svg>
              {logoutLoading ? (
                <>
                  <MySpinner /> <span>Logging out...</span>
                </>
              ) : (
                'Logout'
              )}
            </Button>
          </div>
        </Container>
      </div>
      <Container>
        <div className='text-start'>
          <Button
            variant={`${
              location.pathname === '/show-invoice' ? 'light' : 'outline-light'
            }`}
            className='rubik-400 border-0 me-1 my-1 rounded-0'
            size='sm'
            onClick={() => navigate('/show-invoice')}
          >
            <svg
              xmlns='http://www.w3.org/2000/svg'
              width='16'
              height='16'
              fill='currentColor'
              className='bi bi-receipt me-2'
              viewBox='0 0 16 16'
            >
              <path d='M1.92.506a.5.5 0 0 1 .434.14L3 1.293l.646-.647a.5.5 0 0 1 .708 0L5 1.293l.646-.647a.5.5 0 0 1 .708 0L7 1.293l.646-.647a.5.5 0 0 1 .708 0L9 1.293l.646-.647a.5.5 0 0 1 .708 0l.646.647.646-.647a.5.5 0 0 1 .708 0l.646.647.646-.647a.5.5 0 0 1 .801.13l.5 1A.5.5 0 0 1 15 2v12a.5.5 0 0 1-.053.224l-.5 1a.5.5 0 0 1-.8.13L13 14.707l-.646.647a.5.5 0 0 1-.708 0L11 14.707l-.646.647a.5.5 0 0 1-.708 0L9 14.707l-.646.647a.5.5 0 0 1-.708 0L7 14.707l-.646.647a.5.5 0 0 1-.708 0L5 14.707l-.646.647a.5.5 0 0 1-.708 0L3 14.707l-.646.647a.5.5 0 0 1-.801-.13l-.5-1A.5.5 0 0 1 1 14V2a.5.5 0 0 1 .053-.224l.5-1a.5.5 0 0 1 .367-.27zm.217 1.338L2 2.118v11.764l.137.274.51-.51a.5.5 0 0 1 .707 0l.646.647.646-.646a.5.5 0 0 1 .708 0l.646.646.646-.646a.5.5 0 0 1 .708 0l.646.646.646-.646a.5.5 0 0 1 .708 0l.646.646.646-.646a.5.5 0 0 1 .708 0l.646.646.646-.646a.5.5 0 0 1 .708 0l.509.509.137-.274V2.118l-.137-.274-.51.51a.5.5 0 0 1-.707 0L12 1.707l-.646.647a.5.5 0 0 1-.708 0L10 1.707l-.646.647a.5.5 0 0 1-.708 0L8 1.707l-.646.647a.5.5 0 0 1-.708 0L6 1.707l-.646.647a.5.5 0 0 1-.708 0L4 1.707l-.646.647a.5.5 0 0 1-.708 0l-.509-.51z' />
              <path d='M3 4.5a.5.5 0 0 1 .5-.5h6a.5.5 0 1 1 0 1h-6a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h6a.5.5 0 1 1 0 1h-6a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h6a.5.5 0 1 1 0 1h-6a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h6a.5.5 0 0 1 0 1h-6a.5.5 0 0 1-.5-.5zm8-6a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 0 1h-1a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 0 1h-1a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 0 1h-1a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 0 1h-1a.5.5 0 0 1-.5-.5z' />
            </svg>
            Show Invoice
          </Button>
          <Button
            variant={`${
              location.pathname === '/personal-details'
                ? 'light'
                : 'outline-light'
            }`}
            className='rubik-400 me-1 my-1 rounded-0 border-0'
            size='sm'
            onClick={() => navigate('/personal-details')}
          >
            <svg
              xmlns='http://www.w3.org/2000/svg'
              width='16'
              height='16'
              fill='currentColor'
              className='bi bi-person-fill me-2'
              viewBox='0 0 16 16'
            >
              <path d='M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H3zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6z' />
            </svg>
            Personal Details
          </Button>
          <Button
            variant={`${
              location.pathname === '/fica-documents'
                ? 'light'
                : 'outline-light'
            }`}
            className='rubik-400 me-1 my-1 rounded-0 border-0'
            size='sm'
            onClick={() => navigate('/fica-documents')}
          >
            <svg
              xmlns='http://www.w3.org/2000/svg'
              width='16'
              height='16'
              fill='currentColor'
              className='bi bi-card-text me-2'
              viewBox='0 0 16 16'
            >
              <path d='M14.5 3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-13a.5.5 0 0 1-.5-.5v-9a.5.5 0 0 1 .5-.5h13zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z' />
              <path d='M3 5.5a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5zM3 8a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9A.5.5 0 0 1 3 8zm0 2.5a.5.5 0 0 1 .5-.5h6a.5.5 0 0 1 0 1h-6a.5.5 0 0 1-.5-.5z' />
            </svg>
            Fica Documents
          </Button>{' '}
          <Button
            variant={`${
              location.pathname === '/credit-note' ? 'light' : 'outline-light'
            }`}
            className='rubik-400 me-1 my-1 rounded-0 border-0'
            size='sm'
            onClick={() => navigate('/credit-note')}
          >
            <svg
              xmlns='http://www.w3.org/2000/svg'
              width='16'
              height='16'
              fill='currentColor'
              className='bi bi-card-text me-2'
              viewBox='0 0 16 16'
            >
              <path d='M14.5 3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-13a.5.5 0 0 1-.5-.5v-9a.5.5 0 0 1 .5-.5h13zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z' />
              <path d='M3 5.5a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5zM3 8a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9A.5.5 0 0 1 3 8zm0 2.5a.5.5 0 0 1 .5-.5h6a.5.5 0 0 1 0 1h-6a.5.5 0 0 1-.5-.5z' />
            </svg>
            Credit Note
          </Button>
          <Button
            variant={`${
              location.pathname === '/payouts' ? 'light' : 'outline-light'
            }`}
            className='rubik-400 me-1 my-1 rounded-0 border-0'
            size='sm'
            onClick={() => navigate('/payouts')}
          >
            <svg
              xmlns='http://www.w3.org/2000/svg'
              width='16'
              height='16'
              fill='currentColor'
              className='bi bi-cash-coin me-2'
              viewBox='0 0 16 16'
            >
              <path
                fill-rule='evenodd'
                d='M11 15a4 4 0 1 0 0-8 4 4 0 0 0 0 8zm5-4a5 5 0 1 1-10 0 5 5 0 0 1 10 0z'
              />
              <path d='M9.438 11.944c.047.596.518 1.06 1.363 1.116v.44h.375v-.443c.875-.061 1.386-.529 1.386-1.207 0-.618-.39-.936-1.09-1.1l-.296-.07v-1.2c.376.043.614.248.671.532h.658c-.047-.575-.54-1.024-1.329-1.073V8.5h-.375v.45c-.747.073-1.255.522-1.255 1.158 0 .562.378.92 1.007 1.066l.248.061v1.272c-.384-.058-.639-.27-.696-.563h-.668zm1.36-1.354c-.369-.085-.569-.26-.569-.522 0-.294.216-.514.572-.578v1.1h-.003zm.432.746c.449.104.655.272.655.569 0 .339-.257.571-.709.614v-1.195l.054.012z' />
              <path d='M1 0a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h4.083c.058-.344.145-.678.258-1H3a2 2 0 0 0-2-2V3a2 2 0 0 0 2-2h10a2 2 0 0 0 2 2v3.528c.38.34.717.728 1 1.154V1a1 1 0 0 0-1-1H1z' />
              <path d='M9.998 5.083 10 5a2 2 0 1 0-3.132 1.65 5.982 5.982 0 0 1 3.13-1.567z' />
            </svg>
            Payouts
          </Button>
          <Button
            variant={`${
              location.pathname === '/electronic-wallet'
                ? 'light'
                : 'outline-light'
            }`}
            className='rubik-400 me-1 my-1 rounded-0 border-0'
            size='sm'
            onClick={() => navigate('/electronic-wallet')}
          >
            <svg
              xmlns='http://www.w3.org/2000/svg'
              width='16'
              height='16'
              fill='currentColor'
              className='bi bi-wallet2 me-2'
              viewBox='0 0 16 16'
            >
              <path d='M12.136.326A1.5 1.5 0 0 1 14 1.78V3h.5A1.5 1.5 0 0 1 16 4.5v9a1.5 1.5 0 0 1-1.5 1.5h-13A1.5 1.5 0 0 1 0 13.5v-9a1.5 1.5 0 0 1 1.432-1.499L12.136.326zM5.562 3H13V1.78a.5.5 0 0 0-.621-.484L5.562 3zM1.5 4a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5h-13z' />
            </svg>{' '}
            Electronic Wallet
          </Button>
        </div>
      </Container>
      <div className='gradient-line my-4' style={{ height: '3px' }}></div>
    </div>
  )
}

export default TopButtons
